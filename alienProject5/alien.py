...
class AlienFleet:
    ...
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/level{m}alien{n}.png'), 0, 1) for n in range(2)] for m in range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/level3alien{n}.png'), 0, 1) for n in range(2)]
    ...
    def __init__(self, game, v=Vector(1, 0)):
        ...
        #         lasers=)
        self.alien_h, self.alien_w = self.settings.alien_wh
        self.lasers = Lasers(game=game, owner=self, alien_fleet=self)
        self.create_fleet()
    ...
    def create_alien(self, row, col):
        ...
        alien = Alien(game=self.game, sound=self.sound, alien_index = row // 2,
                     ul=(x, y), v=self.v, image_list=images, lasers=self.lasers)
...
class Alien(Sprite):
    def __init__(self, game, image_list, alien_index, sound, lasers, start_index=0,
                 ul=(0, 100), v=Vector(1, 0), points=1211):
        super().__init__()
        self.game = game
        ...
        self.image = pg.image.load('images/level0alien0.png')
        self.screen_rect = self.screen.get_rect()
    ...
    def update(self, delta_s=Vector(0, 0)):
        ...
        if randint(0, 10000) == 100:
            self.lasers.fire(copy(self.ul), Vector(0, 1))

    def draw(self):
      image = self.timer.image()
      rect = image.get_rect()
      rect.x, rect.y = self.rect.x, self.rect.y
      if not self.lasers == None:
          self.lasers.draw()
